...
$t0104_daftarsiswamaster_edit = NULL; // Initialize page object first

class ct0104_daftarsiswamaster_edit extends ct0104_daftarsiswamaster {
	...
	// Page object name
	var $PageObjName = 't0104_daftarsiswamaster_edit';
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("t0104_daftarsiswamasterlist.php"));
			else
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("t0104_daftarsiswamasterlist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("t0104_daftarsiswamasterlist.php"); // No matching record, return to list
				}
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "t0104_daftarsiswamasterlist.php")
	...
	function RenderRow() {
		...
		$this->kelas_id->ViewCustomAttributes = "";
			...
			$sSqlWrk = "SELECT `id`, `Sekolah` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `t0102_sekolah`";
			$sWhereWrk = "";
			$this->sekolah_id->LookupFilters = array();
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			$this->Lookup_Selecting($this->sekolah_id, $sWhereWrk); // Call Lookup selecting
			...
			$sSqlWrk = "SELECT `id`, `Kelas` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `t0103_kelas`";
			$sWhereWrk = "";
			$this->kelas_id->LookupFilters = array();
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			$this->Lookup_Selecting($this->kelas_id, $sWhereWrk); // Call Lookup selecting
	...
	function ValidateForm() {
		...
		if (in_array("t0105_daftarsiswadetail", $DetailTblVar) && $GLOBALS["t0105_daftarsiswadetail"]->DetailEdit) {
			if (!isset($GLOBALS["t0105_daftarsiswadetail_grid"])) $GLOBALS["t0105_daftarsiswadetail_grid"] = new ct0105_daftarsiswadetail_grid(); // get detail page object
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("t0105_daftarsiswadetail", $DetailTblVar)) {
				if (!isset($GLOBALS["t0105_daftarsiswadetail_grid"]))
					$GLOBALS["t0105_daftarsiswadetail_grid"] = new ct0105_daftarsiswadetail_grid;
				if ($GLOBALS["t0105_daftarsiswadetail_grid"]->DetailEdit) {
					...
					$GLOBALS["t0105_daftarsiswadetail_grid"]->daftarsiswamaster_id->FldIsDetailKey = TRUE;
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x_sekolah_id":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id` AS `LinkFld`, `Sekolah` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `t0102_sekolah`";
			$sWhereWrk = "";
			$this->sekolah_id->LookupFilters = array();
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "", "f0" => '`id` = {filter_value}', "t0" => "3", "fn0" => "");
			$sSqlWrk = "";
		...
		case "x_kelas_id":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id` AS `LinkFld`, `Kelas` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `t0103_kelas`";
			$sWhereWrk = "";
			$this->kelas_id->LookupFilters = array();
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "", "f0" => '`id` = {filter_value}', "t0" => "3", "fn0" => "");
			$sSqlWrk = "";
...
// Create page object
if (!isset($t0104_daftarsiswamaster_edit)) $t0104_daftarsiswamaster_edit = new ct0104_daftarsiswamaster_edit();

// Page init
$t0104_daftarsiswamaster_edit->Page_Init();

// Page main
$t0104_daftarsiswamaster_edit->Page_Main();
...
var CurrentForm = ft0104_daftarsiswamasteredit = new ew_Form("ft0104_daftarsiswamasteredit", "edit");
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t0104_daftarsiswamaster->tahunajaran_id->FldCaption(), $t0104_daftarsiswamaster->tahunajaran_id->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_sekolah_id");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t0104_daftarsiswamaster->sekolah_id->FldCaption(), $t0104_daftarsiswamaster->sekolah_id->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_kelas_id");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t0104_daftarsiswamaster->kelas_id->FldCaption(), $t0104_daftarsiswamaster->kelas_id->ReqErrMsg)) ?>");
...
ft0104_daftarsiswamasteredit.Form_CustomValidate = 
...
ft0104_daftarsiswamasteredit.ValidateRequired = true;
<?php } else { ?>
ft0104_daftarsiswamasteredit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft0104_daftarsiswamasteredit.Lists["x_tahunajaran_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_TahunAjaran","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t0101_tahunajaran"};
ft0104_daftarsiswamasteredit.Lists["x_sekolah_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_Sekolah","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t0102_sekolah"};
ft0104_daftarsiswamasteredit.Lists["x_kelas_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_Kelas","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t0103_kelas"};
...
?>
<form name="ft0104_daftarsiswamasteredit" id="ft0104_daftarsiswamasteredit" class="<?php echo $t0104_daftarsiswamaster_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t0104_daftarsiswamaster_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t0104_daftarsiswamaster_edit->Token ?>">
<?php } ?>
...
<?php if ($t0104_daftarsiswamaster->tahunajaran_id->Visible) { // tahunajaran_id ?>
	<div id="r_tahunajaran_id" class="form-group">
		<label id="elh_t0104_daftarsiswamaster_tahunajaran_id" for="x_tahunajaran_id" class="col-sm-2 control-label ewLabel"><?php echo $t0104_daftarsiswamaster->tahunajaran_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t0104_daftarsiswamaster->tahunajaran_id->CellAttributes() ?>>
<span id="el_t0104_daftarsiswamaster_tahunajaran_id">
<select data-table="t0104_daftarsiswamaster" data-field="x_tahunajaran_id" data-value-separator="<?php echo $t0104_daftarsiswamaster->tahunajaran_id->DisplayValueSeparatorAttribute() ?>" id="x_tahunajaran_id" name="x_tahunajaran_id"<?php echo $t0104_daftarsiswamaster->tahunajaran_id->EditAttributes() ?>>
<?php echo $t0104_daftarsiswamaster->tahunajaran_id->SelectOptionListHtml("x_tahunajaran_id") ?>
</select>
<input type="hidden" name="s_x_tahunajaran_id" id="s_x_tahunajaran_id" value="<?php echo $t0104_daftarsiswamaster->tahunajaran_id->LookupFilterQuery() ?>">
</span>
...
<?php if ($t0104_daftarsiswamaster->sekolah_id->Visible) { // sekolah_id ?>
	<div id="r_sekolah_id" class="form-group">
		<label id="elh_t0104_daftarsiswamaster_sekolah_id" for="x_sekolah_id" class="col-sm-2 control-label ewLabel"><?php echo $t0104_daftarsiswamaster->sekolah_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t0104_daftarsiswamaster->sekolah_id->CellAttributes() ?>>
<span id="el_t0104_daftarsiswamaster_sekolah_id">
<select data-table="t0104_daftarsiswamaster" data-field="x_sekolah_id" data-value-separator="<?php echo $t0104_daftarsiswamaster->sekolah_id->DisplayValueSeparatorAttribute() ?>" id="x_sekolah_id" name="x_sekolah_id"<?php echo $t0104_daftarsiswamaster->sekolah_id->EditAttributes() ?>>
<?php echo $t0104_daftarsiswamaster->sekolah_id->SelectOptionListHtml("x_sekolah_id") ?>
</select>
<input type="hidden" name="s_x_sekolah_id" id="s_x_sekolah_id" value="<?php echo $t0104_daftarsiswamaster->sekolah_id->LookupFilterQuery() ?>">
</span>
...
<?php if ($t0104_daftarsiswamaster->kelas_id->Visible) { // kelas_id ?>
	<div id="r_kelas_id" class="form-group">
		<label id="elh_t0104_daftarsiswamaster_kelas_id" for="x_kelas_id" class="col-sm-2 control-label ewLabel"><?php echo $t0104_daftarsiswamaster->kelas_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t0104_daftarsiswamaster->kelas_id->CellAttributes() ?>>
<span id="el_t0104_daftarsiswamaster_kelas_id">
<select data-table="t0104_daftarsiswamaster" data-field="x_kelas_id" data-value-separator="<?php echo $t0104_daftarsiswamaster->kelas_id->DisplayValueSeparatorAttribute() ?>" id="x_kelas_id" name="x_kelas_id"<?php echo $t0104_daftarsiswamaster->kelas_id->EditAttributes() ?>>
<?php echo $t0104_daftarsiswamaster->kelas_id->SelectOptionListHtml("x_kelas_id") ?>
</select>
<input type="hidden" name="s_x_kelas_id" id="s_x_kelas_id" value="<?php echo $t0104_daftarsiswamaster->kelas_id->LookupFilterQuery() ?>">
</span>
<?php echo $t0104_daftarsiswamaster->kelas_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-table="t0104_daftarsiswamaster" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($t0104_daftarsiswamaster->id->CurrentValue) ?>">
<?php
...
<h4 class="ewDetailCaption"><?php echo $Language->TablePhrase("t0105_daftarsiswadetail", "TblCaption") ?></h4>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t0104_daftarsiswamaster_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft0104_daftarsiswamasteredit.Init();
</script>
<?php
...
<?php
$t0104_daftarsiswamaster_edit->Page_Terminate();
?>
