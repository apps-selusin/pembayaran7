...
$t0302_bayardetail = NULL;

//
// Table class for t0302_bayardetail
//
class ct0302_bayardetail extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('t0302_bayardetail', 't0302_bayardetail', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// bayarmaster_id
		$this->bayarmaster_id = new cField('t0302_bayardetail', 't0302_bayardetail', 'x_bayarmaster_id', 'bayarmaster_id', '`bayarmaster_id`', '`bayarmaster_id`', 3, -1, FALSE, '`bayarmaster_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// iuran_id
		$this->iuran_id = new cField('t0302_bayardetail', 't0302_bayardetail', 'x_iuran_id', 'iuran_id', '`iuran_id`', '`iuran_id`', 3, -1, FALSE, '`iuran_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Periode1
		$this->Periode1 = new cField('t0302_bayardetail', 't0302_bayardetail', 'x_Periode1', 'Periode1', '`Periode1`', '`Periode1`', 200, -1, FALSE, '`Periode1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// Periode2
		$this->Periode2 = new cField('t0302_bayardetail', 't0302_bayardetail', 'x_Periode2', 'Periode2', '`Periode2`', '`Periode2`', 200, -1, FALSE, '`Periode2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// Keterangan
		$this->Keterangan = new cField('t0302_bayardetail', 't0302_bayardetail', 'x_Keterangan', 'Keterangan', '`Keterangan`', '`Keterangan`', 200, -1, FALSE, '`Keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Keterangan->Sortable = TRUE; // Allow sort
		$this->fields['Keterangan'] = &$this->Keterangan;

		// Jumlah
		$this->Jumlah = new cField('t0302_bayardetail', 't0302_bayardetail', 'x_Jumlah', 'Jumlah', '`Jumlah`', '`Jumlah`', 4, -1, FALSE, '`Jumlah`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	// Master filter
	function SqlMasterFilter_t0301_bayarmaster() {
		return "`id`=@id@";
	}

	// Detail filter
	function SqlDetailFilter_t0301_bayarmaster() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t0302_bayardetail`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t0302_bayardetailview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t0302_bayardetailview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t0302_bayardetailadd.php?" . $this->UrlParm($parm);
		else
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t0302_bayardetailedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t0302_bayardetailadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t0302_bayardetaildelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "t0301_bayarmaster" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Periode1->Exportable) $Doc->ExportCaption($this->Periode1);
					if ($this->Periode2->Exportable) $Doc->ExportCaption($this->Periode2);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Periode1->Exportable) $Doc->ExportField($this->Periode1);
						if ($this->Periode2->Exportable) $Doc->ExportField($this->Periode2);
		...
		if($this->PageID == "grid") {
			...
			if(isset($this->RowIndex)) {
				...
				if(($grid_count >= $grid_num) && is_int($grid_num) && ($grid_num >= 1)) {
					...
					// ambil data iuran
					$r = ew_ExecuteRow("select * from t0202_siswaiuran where siswa_id = ".$_GET["siswa_id"]." and tahunajaran_id = ".$_SESSION["tahunajaran_id"]." limit 1 OFFSET $offseter");
