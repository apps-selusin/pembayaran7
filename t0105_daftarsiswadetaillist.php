...
$t0105_daftarsiswadetail_list = NULL; // Initialize page object first

class ct0105_daftarsiswadetail_list extends ct0105_daftarsiswadetail {
	...
	// Page object name
	var $PageObjName = 't0105_daftarsiswadetail_list';

	// Grid form hidden field names
	var $FormName = 'ft0105_daftarsiswadetaillist';
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft0105_daftarsiswadetaillistsrch";
	...
	function Page_Main() {
		...
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t0104_daftarsiswamaster") {
			...
			if (!$this->MasterRecordExists) {
				...
				$this->Page_Terminate("t0104_daftarsiswamasterlist.php"); // Return to master page
			} else {
				$t0104_daftarsiswamaster->LoadListRowValues($rsmaster);
				$t0104_daftarsiswamaster->RowType = EW_ROWTYPE_MASTER; // Master row
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft0105_daftarsiswadetaillistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft0105_daftarsiswadetaillistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ft0105_daftarsiswadetaillist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_t0105_daftarsiswadetail\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_t0105_daftarsiswadetail',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ft0105_daftarsiswadetaillist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t0104_daftarsiswamaster") {
			global $t0104_daftarsiswamaster;
			if (!isset($t0104_daftarsiswamaster)) $t0104_daftarsiswamaster = new ct0104_daftarsiswamaster;
			$rsmaster = $t0104_daftarsiswamaster->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$t0104_daftarsiswamaster;
					$t0104_daftarsiswamaster->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
...
// Create page object
if (!isset($t0105_daftarsiswadetail_list)) $t0105_daftarsiswadetail_list = new ct0105_daftarsiswadetail_list();

// Page init
$t0105_daftarsiswadetail_list->Page_Init();

// Page main
$t0105_daftarsiswadetail_list->Page_Main();
...
var CurrentForm = ft0105_daftarsiswadetaillist = new ew_Form("ft0105_daftarsiswadetaillist", "list");
ft0105_daftarsiswadetaillist.FormKeyCountName = '<?php echo $t0105_daftarsiswadetail_list->FormKeyCountName ?>';

// Form_CustomValidate event
ft0105_daftarsiswadetaillist.Form_CustomValidate = 
...
ft0105_daftarsiswadetaillist.ValidateRequired = true;
<?php } else { ?>
ft0105_daftarsiswadetaillist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft0105_daftarsiswadetaillist.Lists["x_siswa_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_NIS","x_Nama","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t0201_siswa"};
...
<?php } ?>
<?php if (($t0105_daftarsiswadetail->Export == "") || (EW_EXPORT_MASTER_RECORD && $t0105_daftarsiswadetail->Export == "print")) { ?>
<?php
...
?>
<?php include_once "t0104_daftarsiswamastermaster.php" ?>
<?php
	}
}
?>
<?php } ?>
<?php
	$bSelectLimit = $t0105_daftarsiswadetail_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($t0105_daftarsiswadetail_list->TotalRecs <= 0)
			$t0105_daftarsiswadetail_list->TotalRecs = $t0105_daftarsiswadetail->SelectRecordCount();
	} else {
		if (!$t0105_daftarsiswadetail_list->Recordset && ($t0105_daftarsiswadetail_list->Recordset = $t0105_daftarsiswadetail_list->LoadRecordset()))
			$t0105_daftarsiswadetail_list->TotalRecs = $t0105_daftarsiswadetail_list->Recordset->RecordCount();
	}
	$t0105_daftarsiswadetail_list->StartRec = 1;
	if ($t0105_daftarsiswadetail_list->DisplayRecs <= 0 || ($t0105_daftarsiswadetail->Export <> "" && $t0105_daftarsiswadetail->ExportAll)) // Display all records
		$t0105_daftarsiswadetail_list->DisplayRecs = $t0105_daftarsiswadetail_list->TotalRecs;
	if (!($t0105_daftarsiswadetail->Export <> "" && $t0105_daftarsiswadetail->ExportAll))
		$t0105_daftarsiswadetail_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$t0105_daftarsiswadetail_list->Recordset = $t0105_daftarsiswadetail_list->LoadRecordset($t0105_daftarsiswadetail_list->StartRec-1, $t0105_daftarsiswadetail_list->DisplayRecs);

	// Set no record found message
	if ($t0105_daftarsiswadetail->CurrentAction == "" && $t0105_daftarsiswadetail_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$t0105_daftarsiswadetail_list->setWarningMessage(ew_DeniedMsg());
		if ($t0105_daftarsiswadetail_list->SearchWhere == "0=101")
			$t0105_daftarsiswadetail_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$t0105_daftarsiswadetail_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
...
<form name="ft0105_daftarsiswadetaillist" id="ft0105_daftarsiswadetaillist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t0105_daftarsiswadetail_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t0105_daftarsiswadetail_list->Token ?>">
<?php } ?>
...
<?php if ($t0105_daftarsiswadetail_list->TotalRecs > 0 || $t0105_daftarsiswadetail->CurrentAction == "gridedit") { ?>
<table id="tbl_t0105_daftarsiswadetaillist" class="table ewTable">
...
// Header row
$t0105_daftarsiswadetail_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($t0105_daftarsiswadetail->siswa_id->Visible) { // siswa_id ?>
	...
	<?php } else { ?>
		<th data-name="siswa_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t0105_daftarsiswadetail->SortUrl($t0105_daftarsiswadetail->siswa_id) ?>',2);"><div id="elh_t0105_daftarsiswadetail_siswa_id" class="t0105_daftarsiswadetail_siswa_id">
...
<tbody>
<?php
if ($t0105_daftarsiswadetail->ExportAll && $t0105_daftarsiswadetail->Export <> "") {
...
if ($t0105_daftarsiswadetail_list->Recordset && !$t0105_daftarsiswadetail_list->Recordset->EOF) {
	...
	$bSelectLimit = $t0105_daftarsiswadetail_list->UseSelectLimit;
	if (!$bSelectLimit && $t0105_daftarsiswadetail_list->StartRec > 1)
...
while ($t0105_daftarsiswadetail_list->RecCnt < $t0105_daftarsiswadetail_list->StopRec) {
	...
	if (intval($t0105_daftarsiswadetail_list->RecCnt) >= intval($t0105_daftarsiswadetail_list->StartRec)) {
...
<a id="<?php echo $t0105_daftarsiswadetail_list->PageObjName . "_row_" . $t0105_daftarsiswadetail_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php
...
<?php if ($t0105_daftarsiswadetail->CurrentAction <> "gridadd" && $t0105_daftarsiswadetail->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t0105_daftarsiswadetail_list->Pager)) $t0105_daftarsiswadetail_list->Pager = new cPrevNextPager($t0105_daftarsiswadetail_list->StartRec, $t0105_daftarsiswadetail_list->DisplayRecs, $t0105_daftarsiswadetail_list->TotalRecs) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t0105_daftarsiswadetail_list->PageUrl() ?>start=<?php echo $t0105_daftarsiswadetail_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t0105_daftarsiswadetail_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t0105_daftarsiswadetail_list->PageUrl() ?>start=<?php echo $t0105_daftarsiswadetail_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t0105_daftarsiswadetail_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t0105_daftarsiswadetail_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t0105_daftarsiswadetail_list->PageUrl() ?>start=<?php echo $t0105_daftarsiswadetail_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t0105_daftarsiswadetail_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t0105_daftarsiswadetail_list->PageUrl() ?>start=<?php echo $t0105_daftarsiswadetail_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<?php } ?>
<?php if ($t0105_daftarsiswadetail_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $t0105_daftarsiswadetail_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="t0105_daftarsiswadetail">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($t0105_daftarsiswadetail_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($t0105_daftarsiswadetail_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($t0105_daftarsiswadetail_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($t0105_daftarsiswadetail_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="200"<?php if ($t0105_daftarsiswadetail_list->DisplayRecs == 200) { ?> selected<?php } ?>>200</option>
<option value="ALL"<?php if ($t0105_daftarsiswadetail->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($t0105_daftarsiswadetail_list->OtherOptions as &$option)
...
<?php if ($t0105_daftarsiswadetail->Export == "") { ?>
<script type="text/javascript">
ft0105_daftarsiswadetaillist.Init();
</script>
<?php } ?>
<?php
...
<?php
$t0105_daftarsiswadetail_list->Page_Terminate();
?>
