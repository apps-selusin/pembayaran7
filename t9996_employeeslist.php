...
$t9996_employees_list = NULL; // Initialize page object first

class ct9996_employees_list extends ct9996_employees {
	...
	// Table name
	var $TableName = 't9996_employees';

	// Page object name
	var $PageObjName = 't9996_employees_list';

	// Grid form hidden field names
	var $FormName = 'ft9996_employeeslist';
	...
	function __construct() {
		...
		if (!isset($GLOBALS["t9996_employees"]) || get_class($GLOBALS["t9996_employees"]) == "ct9996_employees") {
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 't9996_employees', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft9996_employeeslistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $t9996_employees;
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "ft9996_employeeslistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft9996_employeeslistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft9996_employeeslistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ft9996_employeeslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"ft9996_employeeslistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		$sSqlWrk = "SELECT `userlevelid`, `userlevelname` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `t9997_userlevels`";
		$sWhereWrk = "";
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_t9996_employees\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_t9996_employees',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ft9996_employeeslist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($t9996_employees_list)) $t9996_employees_list = new ct9996_employees_list();

// Page init
$t9996_employees_list->Page_Init();

// Page main
$t9996_employees_list->Page_Main();
...
<?php if ($t9996_employees->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = ft9996_employeeslist = new ew_Form("ft9996_employeeslist", "list");
ft9996_employeeslist.FormKeyCountName = '<?php echo $t9996_employees_list->FormKeyCountName ?>';

// Form_CustomValidate event
ft9996_employeeslist.Form_CustomValidate = 
...
ft9996_employeeslist.ValidateRequired = true;
<?php } else { ?>
ft9996_employeeslist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft9996_employeeslist.Lists["x_UserLevel"] = {"LinkField":"x_userlevelid","Ajax":true,"AutoFill":false,"DisplayFields":["x_userlevelname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t9997_userlevels"};
ft9996_employeeslist.Lists["x_Activated"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft9996_employeeslist.Lists["x_Activated"].Options = <?php echo json_encode($t9996_employees->Activated->Options()) ?>;

// Form object for search
var CurrentSearchForm = ft9996_employeeslistsrch = new ew_Form("ft9996_employeeslistsrch");
...
// Form_CustomValidate event
ft9996_employeeslistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
<?php if (EW_CLIENT_VALIDATE) { ?>
ft9996_employeeslistsrch.ValidateRequired = true; // Use JavaScript validation
<?php } else { ?>
ft9996_employeeslistsrch.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Dynamic selection lists
ft9996_employeeslistsrch.Lists["x_Activated"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft9996_employeeslistsrch.Lists["x_Activated"].Options = <?php echo json_encode($t9996_employees->Activated->Options()) ?>;
</script>
<script type="text/javascript">
...
<?php } ?>
<?php
	$bSelectLimit = $t9996_employees_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($t9996_employees_list->TotalRecs <= 0)
			$t9996_employees_list->TotalRecs = $t9996_employees->SelectRecordCount();
	} else {
		if (!$t9996_employees_list->Recordset && ($t9996_employees_list->Recordset = $t9996_employees_list->LoadRecordset()))
			$t9996_employees_list->TotalRecs = $t9996_employees_list->Recordset->RecordCount();
	}
	$t9996_employees_list->StartRec = 1;
	if ($t9996_employees_list->DisplayRecs <= 0 || ($t9996_employees->Export <> "" && $t9996_employees->ExportAll)) // Display all records
		$t9996_employees_list->DisplayRecs = $t9996_employees_list->TotalRecs;
	if (!($t9996_employees->Export <> "" && $t9996_employees->ExportAll))
		$t9996_employees_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$t9996_employees_list->Recordset = $t9996_employees_list->LoadRecordset($t9996_employees_list->StartRec-1, $t9996_employees_list->DisplayRecs);

	// Set no record found message
	if ($t9996_employees->CurrentAction == "" && $t9996_employees_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$t9996_employees_list->setWarningMessage(ew_DeniedMsg());
		if ($t9996_employees_list->SearchWhere == "0=101")
			$t9996_employees_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$t9996_employees_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$t9996_employees_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($t9996_employees->Export == "" && $t9996_employees->CurrentAction == "") { ?>
<form name="ft9996_employeeslistsrch" id="ft9996_employeeslistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($t9996_employees_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="ft9996_employeeslistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
// Render for search
$t9996_employees->RowType = EW_ROWTYPE_SEARCH;

// Render row
$t9996_employees->ResetAttrs();
...
<?php if ($t9996_employees->Activated->Visible) { // Activated ?>
	<div id="xsc_Activated" class="ewCell form-group">
		<label class="ewSearchCaption ewLabel"><?php echo $t9996_employees->Activated->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_Activated" id="z_Activated" value="="></span>
		<span class="ewSearchField">
<div id="tp_x_Activated" class="ewTemplate"><input type="radio" data-table="t9996_employees" data-field="x_Activated" data-value-separator="<?php echo $t9996_employees->Activated->DisplayValueSeparatorAttribute() ?>" name="x_Activated" id="x_Activated" value="{value}"<?php echo $t9996_employees->Activated->EditAttributes() ?>></div>
<div id="dsl_x_Activated" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $t9996_employees->Activated->RadioButtonListHtml(FALSE, "x_Activated") ?>
</div></div>
</span>
	</div>
<?php } ?>
...
<div id="xsr_2" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($t9996_employees_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($t9996_employees_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $t9996_employees_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($t9996_employees_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($t9996_employees_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($t9996_employees_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($t9996_employees_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<div class="panel panel-default ewGrid t9996_employees">
<form name="ft9996_employeeslist" id="ft9996_employeeslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t9996_employees_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t9996_employees_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t9996_employees">
<div id="gmp_t9996_employees" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($t9996_employees_list->TotalRecs > 0 || $t9996_employees->CurrentAction == "gridedit") { ?>
<table id="tbl_t9996_employeeslist" class="table ewTable">
<?php echo $t9996_employees->TableCustomInnerHtml ?>
<thead><!-- Table header -->
	<tr class="ewTableHeader">
<?php

// Header row
$t9996_employees_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($t9996_employees->EmployeeID->Visible) { // EmployeeID ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->EmployeeID) == "") { ?>
		<th data-name="EmployeeID"><div id="elh_t9996_employees_EmployeeID" class="t9996_employees_EmployeeID"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->EmployeeID->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="EmployeeID"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->EmployeeID) ?>',2);"><div id="elh_t9996_employees_EmployeeID" class="t9996_employees_EmployeeID">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->EmployeeID->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->EmployeeID->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->EmployeeID->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->LastName->Visible) { // LastName ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->LastName) == "") { ?>
		<th data-name="LastName"><div id="elh_t9996_employees_LastName" class="t9996_employees_LastName"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->LastName->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="LastName"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->LastName) ?>',2);"><div id="elh_t9996_employees_LastName" class="t9996_employees_LastName">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->LastName->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->LastName->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->LastName->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->FirstName->Visible) { // FirstName ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->FirstName) == "") { ?>
		<th data-name="FirstName"><div id="elh_t9996_employees_FirstName" class="t9996_employees_FirstName"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->FirstName->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="FirstName"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->FirstName) ?>',2);"><div id="elh_t9996_employees_FirstName" class="t9996_employees_FirstName">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->FirstName->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->FirstName->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->FirstName->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->Title->Visible) { // Title ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->Title) == "") { ?>
		<th data-name="Title"><div id="elh_t9996_employees_Title" class="t9996_employees_Title"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->Title->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Title"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->Title) ?>',2);"><div id="elh_t9996_employees_Title" class="t9996_employees_Title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->Title->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->Title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->Title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->TitleOfCourtesy->Visible) { // TitleOfCourtesy ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->TitleOfCourtesy) == "") { ?>
		<th data-name="TitleOfCourtesy"><div id="elh_t9996_employees_TitleOfCourtesy" class="t9996_employees_TitleOfCourtesy"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->TitleOfCourtesy->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="TitleOfCourtesy"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->TitleOfCourtesy) ?>',2);"><div id="elh_t9996_employees_TitleOfCourtesy" class="t9996_employees_TitleOfCourtesy">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->TitleOfCourtesy->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->TitleOfCourtesy->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->TitleOfCourtesy->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->BirthDate->Visible) { // BirthDate ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->BirthDate) == "") { ?>
		<th data-name="BirthDate"><div id="elh_t9996_employees_BirthDate" class="t9996_employees_BirthDate"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->BirthDate->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="BirthDate"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->BirthDate) ?>',2);"><div id="elh_t9996_employees_BirthDate" class="t9996_employees_BirthDate">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->BirthDate->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->BirthDate->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->BirthDate->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->HireDate->Visible) { // HireDate ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->HireDate) == "") { ?>
		<th data-name="HireDate"><div id="elh_t9996_employees_HireDate" class="t9996_employees_HireDate"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->HireDate->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="HireDate"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->HireDate) ?>',2);"><div id="elh_t9996_employees_HireDate" class="t9996_employees_HireDate">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->HireDate->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->HireDate->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->HireDate->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->Address->Visible) { // Address ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->Address) == "") { ?>
		<th data-name="Address"><div id="elh_t9996_employees_Address" class="t9996_employees_Address"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->Address->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Address"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->Address) ?>',2);"><div id="elh_t9996_employees_Address" class="t9996_employees_Address">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->Address->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->Address->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->Address->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->City->Visible) { // City ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->City) == "") { ?>
		<th data-name="City"><div id="elh_t9996_employees_City" class="t9996_employees_City"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->City->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="City"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->City) ?>',2);"><div id="elh_t9996_employees_City" class="t9996_employees_City">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->City->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->City->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->City->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->Region->Visible) { // Region ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->Region) == "") { ?>
		<th data-name="Region"><div id="elh_t9996_employees_Region" class="t9996_employees_Region"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->Region->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Region"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->Region) ?>',2);"><div id="elh_t9996_employees_Region" class="t9996_employees_Region">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->Region->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->Region->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->Region->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->PostalCode->Visible) { // PostalCode ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->PostalCode) == "") { ?>
		<th data-name="PostalCode"><div id="elh_t9996_employees_PostalCode" class="t9996_employees_PostalCode"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->PostalCode->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="PostalCode"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->PostalCode) ?>',2);"><div id="elh_t9996_employees_PostalCode" class="t9996_employees_PostalCode">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->PostalCode->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->PostalCode->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->PostalCode->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->Country->Visible) { // Country ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->Country) == "") { ?>
		<th data-name="Country"><div id="elh_t9996_employees_Country" class="t9996_employees_Country"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->Country->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Country"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->Country) ?>',2);"><div id="elh_t9996_employees_Country" class="t9996_employees_Country">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->Country->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->Country->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->Country->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->HomePhone->Visible) { // HomePhone ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->HomePhone) == "") { ?>
		<th data-name="HomePhone"><div id="elh_t9996_employees_HomePhone" class="t9996_employees_HomePhone"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->HomePhone->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="HomePhone"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->HomePhone) ?>',2);"><div id="elh_t9996_employees_HomePhone" class="t9996_employees_HomePhone">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->HomePhone->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->HomePhone->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->HomePhone->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->Extension->Visible) { // Extension ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->Extension) == "") { ?>
		<th data-name="Extension"><div id="elh_t9996_employees_Extension" class="t9996_employees_Extension"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->Extension->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Extension"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->Extension) ?>',2);"><div id="elh_t9996_employees_Extension" class="t9996_employees_Extension">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->Extension->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->Extension->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->Extension->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->_Email->Visible) { // Email ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->_Email) == "") { ?>
		<th data-name="_Email"><div id="elh_t9996_employees__Email" class="t9996_employees__Email"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->_Email->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="_Email"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->_Email) ?>',2);"><div id="elh_t9996_employees__Email" class="t9996_employees__Email">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->_Email->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->_Email->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->_Email->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->Photo->Visible) { // Photo ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->Photo) == "") { ?>
		<th data-name="Photo"><div id="elh_t9996_employees_Photo" class="t9996_employees_Photo"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->Photo->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Photo"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->Photo) ?>',2);"><div id="elh_t9996_employees_Photo" class="t9996_employees_Photo">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->Photo->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->Photo->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->Photo->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->ReportsTo->Visible) { // ReportsTo ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->ReportsTo) == "") { ?>
		<th data-name="ReportsTo"><div id="elh_t9996_employees_ReportsTo" class="t9996_employees_ReportsTo"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->ReportsTo->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="ReportsTo"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->ReportsTo) ?>',2);"><div id="elh_t9996_employees_ReportsTo" class="t9996_employees_ReportsTo">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->ReportsTo->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->ReportsTo->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->ReportsTo->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->Password->Visible) { // Password ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->Password) == "") { ?>
		<th data-name="Password"><div id="elh_t9996_employees_Password" class="t9996_employees_Password"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->Password->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Password"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->Password) ?>',2);"><div id="elh_t9996_employees_Password" class="t9996_employees_Password">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->Password->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->Password->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->Password->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->UserLevel->Visible) { // UserLevel ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->UserLevel) == "") { ?>
		<th data-name="UserLevel"><div id="elh_t9996_employees_UserLevel" class="t9996_employees_UserLevel"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->UserLevel->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="UserLevel"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->UserLevel) ?>',2);"><div id="elh_t9996_employees_UserLevel" class="t9996_employees_UserLevel">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->UserLevel->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->UserLevel->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->UserLevel->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->Username->Visible) { // Username ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->Username) == "") { ?>
		<th data-name="Username"><div id="elh_t9996_employees_Username" class="t9996_employees_Username"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->Username->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Username"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->Username) ?>',2);"><div id="elh_t9996_employees_Username" class="t9996_employees_Username">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->Username->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->Username->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->Username->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t9996_employees->Activated->Visible) { // Activated ?>
	<?php if ($t9996_employees->SortUrl($t9996_employees->Activated) == "") { ?>
		<th data-name="Activated"><div id="elh_t9996_employees_Activated" class="t9996_employees_Activated"><div class="ewTableHeaderCaption"><?php echo $t9996_employees->Activated->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Activated"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t9996_employees->SortUrl($t9996_employees->Activated) ?>',2);"><div id="elh_t9996_employees_Activated" class="t9996_employees_Activated">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t9996_employees->Activated->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($t9996_employees->Activated->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t9996_employees->Activated->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<tbody>
<?php
if ($t9996_employees->ExportAll && $t9996_employees->Export <> "") {
...
if ($t9996_employees_list->Recordset && !$t9996_employees_list->Recordset->EOF) {
	...
	$bSelectLimit = $t9996_employees_list->UseSelectLimit;
	if (!$bSelectLimit && $t9996_employees_list->StartRec > 1)
...
$t9996_employees->RowType = EW_ROWTYPE_AGGREGATEINIT;
$t9996_employees->ResetAttrs();
$t9996_employees_list->RenderRow();
while ($t9996_employees_list->RecCnt < $t9996_employees_list->StopRec) {
	$t9996_employees_list->RecCnt++;
	if (intval($t9996_employees_list->RecCnt) >= intval($t9996_employees_list->StartRec)) {
		...
		if ($t9996_employees->CurrentAction == "gridadd") {
		} else {
			$t9996_employees_list->LoadRowValues($t9996_employees_list->Recordset); // Load row values
		}
		$t9996_employees->RowType = EW_ROWTYPE_VIEW; // Render view
...
<?php echo $t9996_employees->EmployeeID->ListViewValue() ?></span>
</span>
<a id="<?php echo $t9996_employees_list->PageObjName . "_row_" . $t9996_employees_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($t9996_employees->LastName->Visible) { // LastName ?>
...
<?php echo $t9996_employees->LastName->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->FirstName->Visible) { // FirstName ?>
...
<?php echo $t9996_employees->FirstName->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->Title->Visible) { // Title ?>
...
<?php echo $t9996_employees->Title->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->TitleOfCourtesy->Visible) { // TitleOfCourtesy ?>
...
<?php echo $t9996_employees->TitleOfCourtesy->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->BirthDate->Visible) { // BirthDate ?>
...
<?php echo $t9996_employees->BirthDate->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->HireDate->Visible) { // HireDate ?>
...
<?php echo $t9996_employees->HireDate->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->Address->Visible) { // Address ?>
...
<?php echo $t9996_employees->Address->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->City->Visible) { // City ?>
...
<?php echo $t9996_employees->City->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->Region->Visible) { // Region ?>
...
<?php echo $t9996_employees->Region->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->PostalCode->Visible) { // PostalCode ?>
...
<?php echo $t9996_employees->PostalCode->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->Country->Visible) { // Country ?>
...
<?php echo $t9996_employees->Country->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->HomePhone->Visible) { // HomePhone ?>
...
<?php echo $t9996_employees->HomePhone->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->Extension->Visible) { // Extension ?>
...
<?php echo $t9996_employees->Extension->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->_Email->Visible) { // Email ?>
...
<?php echo $t9996_employees->_Email->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->Photo->Visible) { // Photo ?>
...
<?php echo $t9996_employees->Photo->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->ReportsTo->Visible) { // ReportsTo ?>
...
<?php echo $t9996_employees->ReportsTo->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->Password->Visible) { // Password ?>
...
<?php echo $t9996_employees->Password->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->UserLevel->Visible) { // UserLevel ?>
...
<?php echo $t9996_employees->UserLevel->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->Username->Visible) { // Username ?>
...
<?php echo $t9996_employees->Username->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($t9996_employees->Activated->Visible) { // Activated ?>
...
<?php echo $t9996_employees->Activated->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
<?php if ($t9996_employees->CurrentAction <> "gridadd" && $t9996_employees->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t9996_employees_list->Pager)) $t9996_employees_list->Pager = new cPrevNextPager($t9996_employees_list->StartRec, $t9996_employees_list->DisplayRecs, $t9996_employees_list->TotalRecs) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t9996_employees_list->PageUrl() ?>start=<?php echo $t9996_employees_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t9996_employees_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t9996_employees_list->PageUrl() ?>start=<?php echo $t9996_employees_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t9996_employees_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t9996_employees_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t9996_employees_list->PageUrl() ?>start=<?php echo $t9996_employees_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t9996_employees_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t9996_employees_list->PageUrl() ?>start=<?php echo $t9996_employees_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<?php } ?>
<?php if ($t9996_employees_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $t9996_employees_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="t9996_employees">
...
<option value="100"<?php if ($t9996_employees_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="200"<?php if ($t9996_employees_list->DisplayRecs == 200) { ?> selected<?php } ?>>200</option>
...
<?php
	foreach ($t9996_employees_list->OtherOptions as &$option)
...
ft9996_employeeslistsrch.Init();
ft9996_employeeslist.Init();
</script>
<?php } ?>
<?php
...
<?php
$t9996_employees_list->Page_Terminate();
?>
