...
$t0202_siswaiuran_list = NULL; // Initialize page object first

class ct0202_siswaiuran_list extends ct0202_siswaiuran {
	...
	// Page object name
	var $PageObjName = 't0202_siswaiuran_list';

	// Grid form hidden field names
	var $FormName = 'ft0202_siswaiuranlist';
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft0202_siswaiuranlistsrch";
	...
	function Page_Main() {
		...
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t0201_siswa") {
			...
			if (!$this->MasterRecordExists) {
				...
				$this->Page_Terminate("t0201_siswalist.php"); // Return to master page
			} else {
				$t0201_siswa->LoadListRowValues($rsmaster);
				$t0201_siswa->RowType = EW_ROWTYPE_MASTER; // Master row
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft0202_siswaiuranlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft0202_siswaiuranlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ft0202_siswaiuranlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_t0202_siswaiuran\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_t0202_siswaiuran',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ft0202_siswaiuranlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t0201_siswa") {
			global $t0201_siswa;
			if (!isset($t0201_siswa)) $t0201_siswa = new ct0201_siswa;
			$rsmaster = $t0201_siswa->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$t0201_siswa;
					$t0201_siswa->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
...
// Create page object
if (!isset($t0202_siswaiuran_list)) $t0202_siswaiuran_list = new ct0202_siswaiuran_list();

// Page init
$t0202_siswaiuran_list->Page_Init();

// Page main
$t0202_siswaiuran_list->Page_Main();
...
var CurrentForm = ft0202_siswaiuranlist = new ew_Form("ft0202_siswaiuranlist", "list");
ft0202_siswaiuranlist.FormKeyCountName = '<?php echo $t0202_siswaiuran_list->FormKeyCountName ?>';

// Form_CustomValidate event
ft0202_siswaiuranlist.Form_CustomValidate = 
...
ft0202_siswaiuranlist.ValidateRequired = true;
<?php } else { ?>
ft0202_siswaiuranlist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft0202_siswaiuranlist.Lists["x_tahunajaran_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_TahunAjaran","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t0101_tahunajaran"};
ft0202_siswaiuranlist.Lists["x_iuran_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_Iuran","x_Jenis","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t0106_iuran"};
...
<?php } ?>
<?php if (($t0202_siswaiuran->Export == "") || (EW_EXPORT_MASTER_RECORD && $t0202_siswaiuran->Export == "print")) { ?>
<?php
...
?>
<?php include_once "t0201_siswamaster.php" ?>
<?php
	}
}
?>
<?php } ?>
<?php
	$bSelectLimit = $t0202_siswaiuran_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($t0202_siswaiuran_list->TotalRecs <= 0)
			$t0202_siswaiuran_list->TotalRecs = $t0202_siswaiuran->SelectRecordCount();
	} else {
		if (!$t0202_siswaiuran_list->Recordset && ($t0202_siswaiuran_list->Recordset = $t0202_siswaiuran_list->LoadRecordset()))
			$t0202_siswaiuran_list->TotalRecs = $t0202_siswaiuran_list->Recordset->RecordCount();
	}
	$t0202_siswaiuran_list->StartRec = 1;
	if ($t0202_siswaiuran_list->DisplayRecs <= 0 || ($t0202_siswaiuran->Export <> "" && $t0202_siswaiuran->ExportAll)) // Display all records
		$t0202_siswaiuran_list->DisplayRecs = $t0202_siswaiuran_list->TotalRecs;
	if (!($t0202_siswaiuran->Export <> "" && $t0202_siswaiuran->ExportAll))
		$t0202_siswaiuran_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$t0202_siswaiuran_list->Recordset = $t0202_siswaiuran_list->LoadRecordset($t0202_siswaiuran_list->StartRec-1, $t0202_siswaiuran_list->DisplayRecs);

	// Set no record found message
	if ($t0202_siswaiuran->CurrentAction == "" && $t0202_siswaiuran_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$t0202_siswaiuran_list->setWarningMessage(ew_DeniedMsg());
		if ($t0202_siswaiuran_list->SearchWhere == "0=101")
			$t0202_siswaiuran_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$t0202_siswaiuran_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
...
<form name="ft0202_siswaiuranlist" id="ft0202_siswaiuranlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t0202_siswaiuran_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t0202_siswaiuran_list->Token ?>">
<?php } ?>
...
<?php if ($t0202_siswaiuran_list->TotalRecs > 0 || $t0202_siswaiuran->CurrentAction == "gridedit") { ?>
<table id="tbl_t0202_siswaiuranlist" class="table ewTable">
...
// Header row
$t0202_siswaiuran_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($t0202_siswaiuran->tahunajaran_id->Visible) { // tahunajaran_id ?>
	...
	<?php } else { ?>
		<th data-name="tahunajaran_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t0202_siswaiuran->SortUrl($t0202_siswaiuran->tahunajaran_id) ?>',2);"><div id="elh_t0202_siswaiuran_tahunajaran_id" class="t0202_siswaiuran_tahunajaran_id">
...
<?php if ($t0202_siswaiuran->iuran_id->Visible) { // iuran_id ?>
	...
	<?php } else { ?>
		<th data-name="iuran_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t0202_siswaiuran->SortUrl($t0202_siswaiuran->iuran_id) ?>',2);"><div id="elh_t0202_siswaiuran_iuran_id" class="t0202_siswaiuran_iuran_id">
...
<?php if ($t0202_siswaiuran->Nilai->Visible) { // Nilai ?>
	...
	<?php } else { ?>
		<th data-name="Nilai"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t0202_siswaiuran->SortUrl($t0202_siswaiuran->Nilai) ?>',2);"><div id="elh_t0202_siswaiuran_Nilai" class="t0202_siswaiuran_Nilai">
...
<tbody>
<?php
if ($t0202_siswaiuran->ExportAll && $t0202_siswaiuran->Export <> "") {
...
if ($t0202_siswaiuran_list->Recordset && !$t0202_siswaiuran_list->Recordset->EOF) {
	...
	$bSelectLimit = $t0202_siswaiuran_list->UseSelectLimit;
	if (!$bSelectLimit && $t0202_siswaiuran_list->StartRec > 1)
...
while ($t0202_siswaiuran_list->RecCnt < $t0202_siswaiuran_list->StopRec) {
	...
	if (intval($t0202_siswaiuran_list->RecCnt) >= intval($t0202_siswaiuran_list->StartRec)) {
...
<a id="<?php echo $t0202_siswaiuran_list->PageObjName . "_row_" . $t0202_siswaiuran_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($t0202_siswaiuran->iuran_id->Visible) { // iuran_id ?>
...
<?php if ($t0202_siswaiuran->CurrentAction <> "gridadd" && $t0202_siswaiuran->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t0202_siswaiuran_list->Pager)) $t0202_siswaiuran_list->Pager = new cPrevNextPager($t0202_siswaiuran_list->StartRec, $t0202_siswaiuran_list->DisplayRecs, $t0202_siswaiuran_list->TotalRecs) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t0202_siswaiuran_list->PageUrl() ?>start=<?php echo $t0202_siswaiuran_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t0202_siswaiuran_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t0202_siswaiuran_list->PageUrl() ?>start=<?php echo $t0202_siswaiuran_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t0202_siswaiuran_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t0202_siswaiuran_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t0202_siswaiuran_list->PageUrl() ?>start=<?php echo $t0202_siswaiuran_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t0202_siswaiuran_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t0202_siswaiuran_list->PageUrl() ?>start=<?php echo $t0202_siswaiuran_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<?php } ?>
<?php if ($t0202_siswaiuran_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $t0202_siswaiuran_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="t0202_siswaiuran">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($t0202_siswaiuran_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($t0202_siswaiuran_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($t0202_siswaiuran_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($t0202_siswaiuran_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="200"<?php if ($t0202_siswaiuran_list->DisplayRecs == 200) { ?> selected<?php } ?>>200</option>
<option value="ALL"<?php if ($t0202_siswaiuran->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($t0202_siswaiuran_list->OtherOptions as &$option)
...
<?php
$t0202_siswaiuran_list->Page_Terminate();
?>
