...
$t0104_daftarsiswamaster = NULL;

//
// Table class for t0104_daftarsiswamaster
//
class ct0104_daftarsiswamaster extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('t0104_daftarsiswamaster', 't0104_daftarsiswamaster', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// tahunajaran_id
		$this->tahunajaran_id = new cField('t0104_daftarsiswamaster', 't0104_daftarsiswamaster', 'x_tahunajaran_id', 'tahunajaran_id', '`tahunajaran_id`', '`tahunajaran_id`', 3, -1, FALSE, '`tahunajaran_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// sekolah_id
		$this->sekolah_id = new cField('t0104_daftarsiswamaster', 't0104_daftarsiswamaster', 'x_sekolah_id', 'sekolah_id', '`sekolah_id`', '`sekolah_id`', 3, -1, FALSE, '`sekolah_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// kelas_id
		$this->kelas_id = new cField('t0104_daftarsiswamaster', 't0104_daftarsiswamaster', 'x_kelas_id', 'kelas_id', '`kelas_id`', '`kelas_id`', 3, -1, FALSE, '`kelas_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function GetDetailUrl() {
		...
		if ($this->getCurrentDetailTable() == "t0105_daftarsiswadetail") {
			$sDetailUrl = $GLOBALS["t0105_daftarsiswadetail"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "t0104_daftarsiswamasterlist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t0104_daftarsiswamaster`";
	}
	...
	function Update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE) {
		...
		// Cascade Update detail table 't0105_daftarsiswadetail'
		$bCascadeUpdate = FALSE;
		$rscascade = array();
		if (!is_null($rsold) && (isset($rs['id']) && $rsold['id'] <> $rs['id'])) { // Update detail field 'daftarsiswamaster_id'
			$bCascadeUpdate = TRUE;
			$rscascade['daftarsiswamaster_id'] = $rs['id']; 
		}
		if ($bCascadeUpdate) {
			if (!isset($GLOBALS["t0105_daftarsiswadetail"])) $GLOBALS["t0105_daftarsiswadetail"] = new ct0105_daftarsiswadetail();
			$rswrk = $GLOBALS["t0105_daftarsiswadetail"]->LoadRs("`daftarsiswamaster_id` = " . ew_QuotedValue($rsold['id'], EW_DATATYPE_NUMBER, 'DB')); 
			while ($rswrk && !$rswrk->EOF) {
				$rskey = array();
				$fldname = 'id';
				$rskey[$fldname] = $rswrk->fields[$fldname];
				$bUpdate = $GLOBALS["t0105_daftarsiswadetail"]->Update($rscascade, $rskey, $rswrk->fields);
	...
	function Delete(&$rs, $where = "", $curfilter = TRUE) {
		...
		// Cascade delete detail table 't0105_daftarsiswadetail'
		if (!isset($GLOBALS["t0105_daftarsiswadetail"])) $GLOBALS["t0105_daftarsiswadetail"] = new ct0105_daftarsiswadetail();
		$rscascade = $GLOBALS["t0105_daftarsiswadetail"]->LoadRs("`daftarsiswamaster_id` = " . ew_QuotedValue($rs['id'], EW_DATATYPE_NUMBER, "DB")); 
		while ($rscascade && !$rscascade->EOF) {
			$GLOBALS["t0105_daftarsiswadetail"]->Delete($rscascade->fields);
			$rscascade->MoveNext();
		}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t0104_daftarsiswamasterview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t0104_daftarsiswamasterview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t0104_daftarsiswamasteradd.php?" . $this->UrlParm($parm);
		else
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t0104_daftarsiswamasteredit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t0104_daftarsiswamasteredit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t0104_daftarsiswamasteradd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t0104_daftarsiswamasteradd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t0104_daftarsiswamasterdelete.php", $this->UrlParm());
	}
