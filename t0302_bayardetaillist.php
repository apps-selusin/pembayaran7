...
$t0302_bayardetail_list = NULL; // Initialize page object first

class ct0302_bayardetail_list extends ct0302_bayardetail {
	...
	// Page object name
	var $PageObjName = 't0302_bayardetail_list';

	// Grid form hidden field names
	var $FormName = 'ft0302_bayardetaillist';
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft0302_bayardetaillistsrch";
	...
	function Page_Main() {
		...
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t0301_bayarmaster") {
			...
			if (!$this->MasterRecordExists) {
				...
				$this->Page_Terminate("t0301_bayarmasterlist.php"); // Return to master page
			} else {
				$t0301_bayarmaster->LoadListRowValues($rsmaster);
				$t0301_bayarmaster->RowType = EW_ROWTYPE_MASTER; // Master row
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "ft0302_bayardetaillistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
		...
		$sFilterList = ew_Concat($sFilterList, $this->bayarmaster_id->AdvancedSearch->ToJSON(), ","); // Field bayarmaster_id
		$sFilterList = ew_Concat($sFilterList, $this->iuran_id->AdvancedSearch->ToJSON(), ","); // Field iuran_id
		$sFilterList = ew_Concat($sFilterList, $this->Periode1->AdvancedSearch->ToJSON(), ","); // Field Periode1
		$sFilterList = ew_Concat($sFilterList, $this->Periode2->AdvancedSearch->ToJSON(), ","); // Field Periode2
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->Periode1, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->Periode2, $arKeywords, $type);
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->iuran_id, $bCtrl); // iuran_id
			$this->UpdateSort($this->Periode1, $bCtrl); // Periode1
			$this->UpdateSort($this->Periode2, $bCtrl); // Periode2
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft0302_bayardetaillistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft0302_bayardetaillistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ft0302_bayardetaillist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"ft0302_bayardetaillistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_t0302_bayardetail\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_t0302_bayardetail',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ft0302_bayardetaillist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t0301_bayarmaster") {
			global $t0301_bayarmaster;
			if (!isset($t0301_bayarmaster)) $t0301_bayarmaster = new ct0301_bayarmaster;
			$rsmaster = $t0301_bayarmaster->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$t0301_bayarmaster;
					$t0301_bayarmaster->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
	...
	function Page_Load() {
		...
		if (isset($_GET["siswa_id"]) and isset($_SESSION["tahunajaran_id"])) {
			$this->GridAddRowCount = ew_ExecuteScalar("select count(*) from t0202_siswaiuran where siswa_id = ".$_GET["siswa_id"]." and tahunajaran_id = ".$_SESSION["tahunajaran_id"]."");
		}
	}
...
// Create page object
if (!isset($t0302_bayardetail_list)) $t0302_bayardetail_list = new ct0302_bayardetail_list();

// Page init
$t0302_bayardetail_list->Page_Init();

// Page main
$t0302_bayardetail_list->Page_Main();
...
var CurrentForm = ft0302_bayardetaillist = new ew_Form("ft0302_bayardetaillist", "list");
ft0302_bayardetaillist.FormKeyCountName = '<?php echo $t0302_bayardetail_list->FormKeyCountName ?>';

// Form_CustomValidate event
ft0302_bayardetaillist.Form_CustomValidate = 
...
ft0302_bayardetaillist.ValidateRequired = true;
<?php } else { ?>
ft0302_bayardetaillist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft0302_bayardetaillist.Lists["x_iuran_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_Iuran","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t0106_iuran"};
ft0302_bayardetaillist.Lists["x_Periode1"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft0302_bayardetaillist.Lists["x_Periode1"].Options = <?php echo json_encode($t0302_bayardetail->Periode1->Options()) ?>;
ft0302_bayardetaillist.Lists["x_Periode2"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft0302_bayardetaillist.Lists["x_Periode2"].Options = <?php echo json_encode($t0302_bayardetail->Periode2->Options()) ?>;

// Form object for search
var CurrentSearchForm = ft0302_bayardetaillistsrch = new ew_Form("ft0302_bayardetaillistsrch");
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if (($t0302_bayardetail->Export == "") || (EW_EXPORT_MASTER_RECORD && $t0302_bayardetail->Export == "print")) { ?>
<?php
...
?>
<?php include_once "t0301_bayarmastermaster.php" ?>
<?php
	}
}
?>
<?php } ?>
<?php
	$bSelectLimit = $t0302_bayardetail_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($t0302_bayardetail_list->TotalRecs <= 0)
			$t0302_bayardetail_list->TotalRecs = $t0302_bayardetail->SelectRecordCount();
	} else {
		if (!$t0302_bayardetail_list->Recordset && ($t0302_bayardetail_list->Recordset = $t0302_bayardetail_list->LoadRecordset()))
			$t0302_bayardetail_list->TotalRecs = $t0302_bayardetail_list->Recordset->RecordCount();
	}
	$t0302_bayardetail_list->StartRec = 1;
	if ($t0302_bayardetail_list->DisplayRecs <= 0 || ($t0302_bayardetail->Export <> "" && $t0302_bayardetail->ExportAll)) // Display all records
		$t0302_bayardetail_list->DisplayRecs = $t0302_bayardetail_list->TotalRecs;
	if (!($t0302_bayardetail->Export <> "" && $t0302_bayardetail->ExportAll))
		$t0302_bayardetail_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$t0302_bayardetail_list->Recordset = $t0302_bayardetail_list->LoadRecordset($t0302_bayardetail_list->StartRec-1, $t0302_bayardetail_list->DisplayRecs);

	// Set no record found message
	if ($t0302_bayardetail->CurrentAction == "" && $t0302_bayardetail_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$t0302_bayardetail_list->setWarningMessage(ew_DeniedMsg());
		if ($t0302_bayardetail_list->SearchWhere == "0=101")
			$t0302_bayardetail_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$t0302_bayardetail_list->setWarningMessage($Language->Phrase("NoRecord"));
	}

	// Audit trail on search
	if ($t0302_bayardetail_list->AuditTrailOnSearch && $t0302_bayardetail_list->Command == "search" && !$t0302_bayardetail_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $t0302_bayardetail_list->getSessionWhere();
		$t0302_bayardetail_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
$t0302_bayardetail_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($t0302_bayardetail->Export == "" && $t0302_bayardetail->CurrentAction == "") { ?>
<form name="ft0302_bayardetaillistsrch" id="ft0302_bayardetaillistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($t0302_bayardetail_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="ft0302_bayardetaillistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="t0302_bayardetail">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($t0302_bayardetail_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($t0302_bayardetail_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $t0302_bayardetail_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($t0302_bayardetail_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($t0302_bayardetail_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($t0302_bayardetail_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($t0302_bayardetail_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<form name="ft0302_bayardetaillist" id="ft0302_bayardetaillist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t0302_bayardetail_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t0302_bayardetail_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t0302_bayardetail">
...
<?php if ($t0302_bayardetail_list->TotalRecs > 0 || $t0302_bayardetail->CurrentAction == "gridedit") { ?>
<table id="tbl_t0302_bayardetaillist" class="table ewTable">
...
// Header row
$t0302_bayardetail_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($t0302_bayardetail->iuran_id->Visible) { // iuran_id ?>
	...
	<?php } else { ?>
		<th data-name="iuran_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t0302_bayardetail->SortUrl($t0302_bayardetail->iuran_id) ?>',2);"><div id="elh_t0302_bayardetail_iuran_id" class="t0302_bayardetail_iuran_id">
...
<?php if ($t0302_bayardetail->Periode1->Visible) { // Periode1 ?>
	...
	<?php } else { ?>
		<th data-name="Periode1"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t0302_bayardetail->SortUrl($t0302_bayardetail->Periode1) ?>',2);"><div id="elh_t0302_bayardetail_Periode1" class="t0302_bayardetail_Periode1">
...
<?php if ($t0302_bayardetail->Periode2->Visible) { // Periode2 ?>
	...
	<?php } else { ?>
		<th data-name="Periode2"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t0302_bayardetail->SortUrl($t0302_bayardetail->Periode2) ?>',2);"><div id="elh_t0302_bayardetail_Periode2" class="t0302_bayardetail_Periode2">
...
<?php if ($t0302_bayardetail->Keterangan->Visible) { // Keterangan ?>
	...
	<?php } else { ?>
		<th data-name="Keterangan"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t0302_bayardetail->SortUrl($t0302_bayardetail->Keterangan) ?>',2);"><div id="elh_t0302_bayardetail_Keterangan" class="t0302_bayardetail_Keterangan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t0302_bayardetail->Keterangan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t0302_bayardetail->Keterangan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t0302_bayardetail->Keterangan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t0302_bayardetail->Jumlah->Visible) { // Jumlah ?>
	...
	<?php } else { ?>
		<th data-name="Jumlah"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t0302_bayardetail->SortUrl($t0302_bayardetail->Jumlah) ?>',2);"><div id="elh_t0302_bayardetail_Jumlah" class="t0302_bayardetail_Jumlah">
...
<tbody>
<?php
if ($t0302_bayardetail->ExportAll && $t0302_bayardetail->Export <> "") {
...
if ($t0302_bayardetail_list->Recordset && !$t0302_bayardetail_list->Recordset->EOF) {
	...
	$bSelectLimit = $t0302_bayardetail_list->UseSelectLimit;
	if (!$bSelectLimit && $t0302_bayardetail_list->StartRec > 1)
...
while ($t0302_bayardetail_list->RecCnt < $t0302_bayardetail_list->StopRec) {
	...
	if (intval($t0302_bayardetail_list->RecCnt) >= intval($t0302_bayardetail_list->StartRec)) {
...
<a id="<?php echo $t0302_bayardetail_list->PageObjName . "_row_" . $t0302_bayardetail_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($t0302_bayardetail->Periode1->Visible) { // Periode1 ?>
...
<?php if ($t0302_bayardetail->CurrentAction <> "gridadd" && $t0302_bayardetail->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t0302_bayardetail_list->Pager)) $t0302_bayardetail_list->Pager = new cPrevNextPager($t0302_bayardetail_list->StartRec, $t0302_bayardetail_list->DisplayRecs, $t0302_bayardetail_list->TotalRecs) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t0302_bayardetail_list->PageUrl() ?>start=<?php echo $t0302_bayardetail_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t0302_bayardetail_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t0302_bayardetail_list->PageUrl() ?>start=<?php echo $t0302_bayardetail_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t0302_bayardetail_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t0302_bayardetail_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t0302_bayardetail_list->PageUrl() ?>start=<?php echo $t0302_bayardetail_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t0302_bayardetail_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t0302_bayardetail_list->PageUrl() ?>start=<?php echo $t0302_bayardetail_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<?php } ?>
<?php if ($t0302_bayardetail_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $t0302_bayardetail_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="t0302_bayardetail">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($t0302_bayardetail_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($t0302_bayardetail_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($t0302_bayardetail_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($t0302_bayardetail_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="200"<?php if ($t0302_bayardetail_list->DisplayRecs == 200) { ?> selected<?php } ?>>200</option>
<option value="ALL"<?php if ($t0302_bayardetail->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($t0302_bayardetail_list->OtherOptions as &$option)
...
ft0302_bayardetaillistsrch.Init();
ft0302_bayardetaillist.Init();
</script>
<?php } ?>
<?php
...
<?php
$t0302_bayardetail_list->Page_Terminate();
?>
